...
Return ONLY JSON."""
    ...
    def _stage3_context_builder(self, ctx, knowledge, provider, schema_ddl):
        ...
        sections.append(
            ...
            f"  - 'unit price' / 'price per unit' = ROUND(SUM(AMOUNT / EXCH_RATE) / NULLIF(SUM(QUANTITY), 0), 2) (spend per unit in EUR)\n"
...
- For {provider.upper()}: {"use FETCH FIRST N ROWS ONLY (not LIMIT), no SYSDATE" if provider == "oracle" else "use LIMIT"}
- Use UPPER(col) LIKE UPPER('%value%') for text matching
- Use ROUND(AMOUNT / EXCH_RATE, 2) for currency (EUR)
